...
1) In the discussion you mention prior evidence that conifers have stable water content as psi declines. I would move this to intro, and use it as prior basis for adding a model that tests whether angio and conifers have different (but shared within) slopes. I.e. + Group + 1|Species. You have a model dropping conifers and adding angiosperms, but why not formally test that they are different?
...
# 1. Tidy the model output
# 'tidy' extracts coefficients, standard errors, and t-values
model_results <- tidy(mod_spp_group) 

# 2. Add a column for the Model Formula for your records
model_results <- model_results %>%
  mutate(formula = "ilfm ~ mwp * group + (1 | species)")

# 3. Save to CSV
# Using here() to ensure it goes into your project folder
write.csv(model_results, 
          file = here("results", "figures", "TableSX_gymno_angio.csv"), 
          row.names = FALSE)

# ---------------------------------------------------------
# Optional: If you want JUST the Fixed Effects in a 
# "publication ready" wide format
# ---------------------------------------------------------
