...
##Add in second line to panel B for the conifer only slope: 
...
# Non-conifer grand slope (the goldenrod line)
grand_excl <- slope_draws %>%
  filter(!species %in% exclude_spp) %>%
  group_by(.draw) %>%
  summarise(grand_slope_excl = mean(slope, na.rm = TRUE), .groups = "drop")
...
# NEW: Conifer-only grand slope
grand_conifers <- slope_draws %>%
  filter(species %in% exclude_spp) %>%
  group_by(.draw) %>%
  summarise(grand_slope_conifer = mean(slope, na.rm = TRUE), .groups = "drop")

grand_med_conifer <- median(grand_conifers$grand_slope_conifer)
...
cri_overlap_flag <- slope_draws %>%
  ...
  mutate(
    overlaps_grand_excl =
      !(hi < grand_ci_excl[[1]] | lo > grand_ci_excl[[2]])
  )
# ----------------------------
# 6) Build plotting dfs (shared y ordering)
# ----------------------------
...
pB <- ggplot(plot_right, aes(x = slope, y = species, fill = fill_grp)) +
  geom_vline(xintercept = 0, linetype = 2, linewidth = 0.4) +
  # Original non-conifer grand slope
  geom_vline(xintercept = grand_med_excl, linewidth = 0.8, color = "goldenrod") +
  # NEW: Conifer-only grand slope (added as a dark blue or choose a color you prefer)
  geom_vline(xintercept = grand_med_conifer, linewidth = 0.8, color = "#17154f", linetype = "solid") +
  geom_density_ridges(scale = 2.2, rel_min_height = 0.01, alpha = 0.85, color = NA) +
  scale_fill_manual(values = c("grey60", "black")) +
  coord_cartesian(xlim = xlims) +
...
ggsave(
  filename = here("figures", "species_slope_posteriors_twoPanel_overlapGrandExcl_withConsSlope.png"),
  plot     = p_AB,
  width    = 8,
  height   = 5,
  units    = "in",
