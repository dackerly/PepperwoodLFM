...
#Fig 7. P50 + TLP and model params - all salvia
...
#Fig 7, just psimin ~ LDMC with salvia: 
...
#
# NEW REQUEST:
#  - Panel A: regressions WITH vs WITHOUT Salvia (two lines + sig linetypes + two R²)
#  - Panels B, C, D: single regression (no with/without split; single sig linetype; single R²)
# ============================================================
...
# Two-line colors (A only)
reg_colors_split <- c(
  "With Salvia"    = "darkgrey",
  "Without Salvia" = "black"
)

# One-line color (B/C/D)
reg_color_single <- c("All species" = "black")
...
get_model_stats <- function(df, yvar, xvar) {
  ...
  df1 <- unname(fstat[2])
  df2 <- unname(fstat[3])
  p_model <- pf(fstat[1], df1, df2, lower.tail = FALSE)
...
# ----------------------------
# Panel builder: SPLIT (With vs Without Salvia)  ==> for panel A only
# ----------------------------
build_panel_xy_split <- function(
  xvar, yvar,
  xlab = NULL,
  ylab = NULL,
  xscale = NULL,
  ...
  p <- ggplot() +
    ...
    # regression scales (separate from species colors)
    ggnewscale::new_scale_color() +
    geom_smooth(
      data = reg_df,
      aes(x = .data[[xvar]], y = .data[[yvar]], color = .reg_group, linetype = .sig),
  ...
  if (add_r2) {
    p <- p +
      annotate(
        ...
        label = paste0("R² (with) = ", sprintf("%.2f", st_with$r2)),
        hjust = 1.02, vjust = 1.25, size = 4,
        color = reg_colors_split["With Salvia"]
      ) +
      annotate(
        "text",
        x = Inf, y = Inf,
        label = paste0("R² (without) = ", sprintf("%.2f", st_without$r2)),
        hjust = 1.02, vjust = 2.55, size = 4,
        color = reg_colors_split["Without Salvia"]
      )
  }
...
salvia_df <- q3_df %>% filter(species %in% salvia_spp)
# ----------------------------
# Panel builder: SINGLE regression (no with/without split) ==> for B/C/D
# ----------------------------
build_panel_xy_single <- function(
  xvar, yvar,
  xlab = NULL,
  ylab = NULL,
  xscale = NULL,
  ...
  reg_df <- q3_df %>% mutate(.reg_group = "All species", .sig = sig_all)
  salvia_df <- q3_df %>% filter(species %in% salvia_spp)
  ...
  p <- ggplot() +
    ...
    # salvia rings (still ring them if you want; does NOT create separate regression)
    # geom_point(
    #   data = salvia_df,
    #   aes(x = .data[[xvar]], y = .data[[yvar]], color = species),
    #   shape = 21, fill = NA,
  ...
  if (add_r2) {
    p <- p +
      annotate(
        ...
        hjust = 1.02, vjust = 1.25, size = 4,
        color = reg_color_single["All species"]
      )
  }
...
# ============================
# Panel D: x = ΨTLP (flipped), y = max_lfm (single regression)
#   - hide y labels/text (share with C visually)
# ============================
fig5d <- build_panel_xy_single(
  xvar = "psi_tlp", yvar = "max_lfm",
  xlab = expression(psi[TLP]~"(MPa)"),
  ylab = "Max. Seasonal LFM (%)",
  yscale = scale_y_continuous(
...
# ----------------------------
# Legend pulled once (from A), then placed in a dedicated column
#   (A includes: Species + Dataset (split) + Significance)
# ----------------------------
legend_grob <- cowplot::get_legend(
  fig5a +
    theme(
...
ggsave(
  plot = fig_all_labeled,
  filename = here::here("figures", "fig5_ABCD_equal_panels_legend_topright_A_split_BCD_single.jpg"),
  dpi = 600,
  width = 6,
  height = 8
)
