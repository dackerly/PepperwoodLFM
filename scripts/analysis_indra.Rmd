...
write_csv(tablesx_coefs, here("results", "figures", "tableSX_ints_coefs.csv"))
```
...
# Apply function for study
coef_study <- fit_glm_extract(df1, "study") %>% 
  mutate(group = "study")
```

#####Figure SX (differencest across studies)
```{r, slopes}
# Apply function for sp_site
coef_sp_site <- fit_glm_extract(df1, "sp_site") %>% 
  mutate(group = "sp_site")%>% 
  separate_wider_delim(sp_site, names = c("species", "study"), "_") 

coef_sp_site_dupes <- coef_sp_site %>% 
  group_by(species) %>% 
  filter(n() >= 2)

coef_sp_site %>% 
  ggplot() +
  geom_point(aes(y = intercept, x = slope, shape = study), data = coef_sp_site, color = "black") +
  geom_point(aes(y = intercept, x = slope, color = species), data = coef_sp_site_dupes, shape = c(1), size = 10)+
  geom_smooth(method = "lm", se = F, aes(y = intercept, x = slope, color = species), data = coef_sp_site_dupes)
...
mod1 <- glm(ilfm ~ mwp*species + study, data = df2, family = gaussian)
mod2 <- glm(ilfm ~ mwp*species, data = df2, family = gaussian)
...
concept_df_wide %>% 
ggplot(aes(x = grand_species, y = species, color = spp_code)) +
  geom_abline() +
  geom_point()
...
fig4a <- make_lm_plot(df_plot, "minlfm", "intercept_lfm", "Minimum LFM (%)", "End-of-season Intercept LFM")
...
ggsave(plot =plots_figsx_param_corrs, 
        filename = here::here("figures", "figsx_param_corrs.jpg"), 
       dpi = 600, width = 20, height = 7)
...
pds_q3_df <- pds_q3_df_raw %>% 
  ...
  distinct()
#add in predawns:
q3_df <- merge(q3_df1, pds_q3_df, by = c("species", "study")) 
```
...
plot_all_relationships <- function(data, x_var, y_vars) {
  plots <- lapply(y_vars, function(y_var) {
    ...
    p <- ggplot(data, aes_string(x = x_var, y = y_var)) +
      geom_point(aes(color = species, shape = study_nice, text = species)) +
...
```{r, warning = F}
y_variables <- c("ldmc", "plc50_stem", "psi_tlp", "sla", "min_pd", "max_pd")
# Generate and store plots with psi_tlp as the x-axis
plots <- plot_all_relationships(q3_df, "max_rooting_depth_m", y_variables)
...
plots_legend_int <- cowplot::plot_grid(plots,
                   ...
                   rel_widths = c(1,.275))
plots_legend_int
ggsave(plots_legend_int, here::here("figures", "figsx_spptraits.jpg", dpi = 600))
```
