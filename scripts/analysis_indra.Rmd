...
coef_species <- fit_glm_extract(df, "species") %>% 
  mutate(group = "species")

# Apply function for sp_site
coef_sp_site <- fit_glm_extract(df, "sp_site") %>% 
  mutate(group = "sp_site")

# Apply function for sp_site
coef_sp_study <- fit_glm_extract(df, "study") %>% 
  mutate(group = "study")
...
coef_sp_grand <- fit_glm_extract_grand(df, "species") %>% 
...
df_pred <- df %>%
  left_join(coef_species, by = "species") %>%  # Merge coefficients
  mutate(
