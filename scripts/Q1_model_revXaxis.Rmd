...
df1 <- df %>% 
  ...
  filter(!(study %in% c("SEKI") & tissue_age %in% c("old", "both")))
...
# ---- x-axis standard mapping (-8 on left, 0 to the right) ----
xlim_mwp <- c(-8, 0)   
...
# conceptual figure, in standard space
lfm.sat <- c(0.9, 0.5)
slp <- -0.2
min.MWP <- c(-6, -3)
...
# ---- ggplot version: standard mapping ---------------------
library(dplyr)
library(ggplot2)
library(patchwork)
...
pA <- ggplot(dfB_curve, aes(x = MWP, y = LFM, color = id)) +
  ...
  annotate("text", x = -7.8, y = 3, label = "A", fontface = "bold",
           hjust = 0, vjust = 1, size = 5) +
  coord_cartesian(ylim = c(0,3), xlim = xlim_mwp) +
...
pB <- ggplot() +
  ...
  geom_point(data = dfA_filled,
             aes(x = MWP, y = invLFM, color = id),
             shape = 19, size = 3.5) +
  annotate("text", x = -7.8, y = 3, label = "B", fontface = "bold",
           hjust = 0, vjust = 1, size = 5) +
  coord_cartesian(ylim = c(0,3), xlim = xlim_mwp) +
  scale_color_manual(values = cols, guide = "none") +
...
for (i in seq_along(ilfm.sat.vec)) MWPcrit[i] <- solveForX(c(ilfm.sat.vec[i], slp), 1/0.7)
lfm.sat.vec <- (1 / ilfm.sat.vec) * 100 # Convert fractional LFM to %
plot(MWPcrit, lfm.sat.vec, xlim = xlim_mwp, ylim = c(0, 400),
     type = "l", lwd = 2,
     xlab = "Minimum MWP (MPa)", ylab = "LFM @ MWP=0 (%)")

for (i in seq_along(ilfm.sat.vec)) MWPcrit[i] <- solveForX(c(ilfm.sat.vec[i], slp), 1/0.5)
lfm.sat.vec <- (1 / ilfm.sat.vec) * 100
lines(MWPcrit, lfm.sat.vec, lwd = 1)
...
xy <- inversePlotData(c(min.MWP[1], 0), c(lfm.sat[1], slp))
head(xy)
plot(xy$MWP, xy$LFM * 100, xlim = xlim_mwp, ylim = c(0, 300), xlab = "MWP", ylab = "LFM (%)")
...
plot(xlim_mwp, c(0, 0.03), type = "n", xlab = "MWP", ylab = "1/LFM")
x <- list(); y <- list()
for (i in 1:2) {
  x[[i]] <- c(0, min.MWP[i])
  y[[i]] <- c(lfm.sat[i], max.iLFM[i]) / 100 # Scale 1/LFM
  lines(x[[i]], y[[i]], type = "b", cex = 2, pch = c(1,19))
}

xy1 <- inversePlotData(c(0, min.MWP[1]), c(lfm.sat[1], slp))
xy1$LFM <- xy1$LFM * 100 # Scale LFM to %
xy2 <- inversePlotData(c(0, min.MWP[2]), c(lfm.sat[2], slp))
xy2$LFM <- xy2$LFM * 100 # Scale LFM to %

plot(xlim_mwp, c(0, 300), type = "n", xlab = "MWP", ylab = "LFM (%)")
lines(xy1$MWP, xy1$LFM, type = "l")
points(xy1[c(1,100), c("MWP","LFM")], cex = 2, pch = c(1,19))
lines(xy2$MWP, xy2$LFM, type = "l")
points(xy2[c(1,100), c("MWP","LFM")], cex = 2, pch = c(1,19))
...
xy <- inversePlotData(c(min.MWP[1], 0), c(lfm.sat[1], slp))
xy$LFM <- xy$LFM * 100 # Scale LFM to %
plot(xy$MWP, xy$LFM, xlim = xlim_mwp, ylim = c(0, 300), xlab = "MWP", ylab = "LFM (%)")
...
for (i in 1:2) {
  x[[i]] <- c(0, min.MWP[i])
  y[[i]] <- c(lfm.sat[i], max.iLFM[i]) / 100
  lines(x[[i]], y[[i]], col = cols[i], lwd = 2)
  points(x[[i]][1], y[[i]][1], pch = 21, bg = "white", col = cols[i], cex = 2, lwd = 2)
  points(x[[i]][2], y[[i]][2], pch = 19, col = cols[i], cex = 2)
}
...
dfA_lines <- tibble(
  id = factor(c(1,1, 2,2)),
  MWP = c(0, min.MWP[1], 0, min.MWP[2]),
  invLFM = c(lfm.sat[1], max.iLFM[1], lfm.sat[2], max.iLFM[2]) / 100 # Scaled to match LFM%
)

dfA_open <- dfA_lines %>% group_by(id) %>% slice_head(n = 1) %>% ungroup()
dfA_filled <- dfA_lines %>% group_by(id) %>% slice_tail(n = 1) %>% ungroup()

xy1 <- inversePlotData(c(0, min.MWP[1]), c(lfm.sat[1], slp)) %>% mutate(id = factor(1), LFM = LFM * 100)
xy2 <- inversePlotData(c(0, min.MWP[2]), c(lfm.sat[2], slp)) %>% mutate(id = factor(2), LFM = LFM * 100)
dfB_curve <- bind_rows(xy1, xy2)
...
pA <- ggplot(dfB_curve, aes(x = MWP, y = LFM, color = id)) +
  ...
  geom_point(data = dfB_open,   shape = 21, fill = "white", size = 3.5, stroke = 1.2) +
  geom_point(data = dfB_filled, shape = 19, size = 3.5) +
  annotate("text", x = -7.8, y = 300, label = "A", fontface = "bold",
           hjust = 0, vjust = 1, size = 5) +
  coord_cartesian(ylim = c(0, 300), xlim = xlim_mwp) +
  scale_color_manual(values = cols, guide = "none") +
  labs(x = "Midday Water Potential (MPa)", y = "LFM (%)") +
  base_theme
...
pB <- ggplot() +
  ...
  geom_point(data = dfA_filled,
             aes(x = MWP, y = invLFM, color = id),
             shape = 19, size = 3.5) +
  annotate("text", x = -7.8, y = 0.03, label = "B", fontface = "bold",
           hjust = 0, vjust = 1, size = 5) +
  coord_cartesian(ylim = c(0, 0.03), xlim = xlim_mwp) +
  scale_color_manual(values = cols, guide = "none") +
  labs(x = "Midday Water Potential (MPa)", y = "1/LFM") +
  base_theme
...
plot(MWPcrit, lfm.sat.vec, xlim = xlim_mwp, ylim = c(0, 4),
     type = "l", lwd = 2,
     xlab = "Minimum MWP (MPa)", ylab = "LFM @ MWP=0 (g water / g dry mass)")
...
xy <- inversePlotData(c(min.MWP[1], 0), c(lfm.sat[1], slp))
head(xy)
plot(xy$MWP, xy$LFM, xlim = xlim_mwp, ylim = c(0, 3), xlab = "MWP", ylab = "LFM (g water / g dry mass)")
...
plot(xlim_mwp, c(0, 3), type = "n", xlab = "MWP", ylab = "1/LFM (g dry mass / g water)")
x <- list(); y <- list()
for (i in 1:2) {
  x[[i]] <- c(0, min.MWP[i])
  y[[i]] <- c(lfm.sat[i], max.iLFM[i]) 
...
plot(xlim_mwp, c(0, 3), type = "n", xlab = "MWP", ylab = "LFM (g water / g dry mass)")
lines(xy1$MWP, xy1$LFM, type = "l")
points(xy1[c(1,100), c("MWP","LFM")], cex = 2, pch = c(1,19))
lines(xy2$MWP, xy2$LFM, type = "l")
points(xy2[c(1,100), c("MWP","LFM")], cex = 2, pch = c(1,19))
...
pA <- ggplot(dfB_curve, aes(x = MWP, y = LFM, color = id)) +
  ...
  coord_cartesian(ylim = c(0, 3), xlim = xlim_mwp) +
  scale_color_manual(values = cols, guide = "none") +
  labs(x = "Midday Water Potential (MPa)", y = expression("LFM (g"["water"] ~ "/ g"["dry mass"] * ")")) +
  base_theme
...
pB <- ggplot() +
  ...
  coord_cartesian(ylim = c(0, 3), xlim = xlim_mwp) +
  scale_color_manual(values = cols, guide = "none") +
  labs(x = "Midday Water Potential (MPa)", y = expression("1/LFM (g"["dry mass"] ~ "/ g"["water"] * ")")) +
  base_theme
...
library(emmeans)
library(performance)
#install.packages(c("pbkrtest", "lmerTest"))
library(lmerTest)
library(pbkrtest)
...
write_csv(n_by_species_study, here("data", "study_species_n.csv"))
...
```
#Table S2: Are models for gymnos and angios different?
...
df2 <- df1 %>% 
  mutate(group = case_when(
    species %in% c("PSEMEN", "PINJEF", "CALDEC", "PINJEF", "ABICON") ~ "Gymnosperm",
    TRUE~ as.character("Angiosperm")
  ))

mod_spp_group <- lmer(ilfm ~ mwp * group + (1|species),
                      data = df2, REML = T)

summary(mod_spp_group)

library(broom.mixed)
library(dplyr)
library(here)

model_results <- tidy(mod_spp_group) 

model_results <- model_results %>%
  mutate(formula = "ilfm ~ mwp * group + (1 | species)")

write.csv(model_results, 
          file = here("results", "figures", "TableSX_gymno_angio.csv"), 
          row.names = FALSE)

fixed_effects_wide <- tidy(mod_spp_group, effects = "fixed") %>%
  select(term, estimate, std.error, statistic)

write.csv(fixed_effects_wide, 
          file = here("results", "figures", "TableSX_wide_gymno_angio.csv"), 
          row.names = FALSE)
...
```{r, predict}
df_pred <- df1 %>%
  left_join(coefs_species_int, by = "species") %>%  
  mutate(
    mwp_pred = (ilfm - intercept) / slope,  
    lfm_pred = 1/(slope * mwp + intercept)  
  ) 
...
```{r, predict}
df_pred <- df1 %>%
  left_join(coefs_species_study, by = "species") %>%  
  mutate(
    mwp_pred = (ilfm - intercept) / slope,  
    lfm_pred = 1/(slope * mwp + intercept)  
  ) 
...
```{r, predict}
df_pred <- df1 %>%
  left_join(coefs_common, by = "species") %>%  
  mutate(
    mwp_pred = (ilfm - intercept) / slope,  
    lfm_pred = 1/(slope * mwp + intercept)  
  ) 
  ...
  left_join(
    concept_slopes_df %>%
      ...
      summarise(
        ...
        abs_max = max(abs(mwp), na.rm = TRUE),   
        sign_mwp = sign(stats::median(mwp, na.rm = TRUE)),
        .groups = "drop"
      ) %>%
      mutate(sign_mwp = if_else(sign_mwp == 0, -1, sign_mwp)), 
    by = "species"
  ) %>%
  rowwise() %>%
...
figSXa <- ggplot(concept_slopes_df , aes(x = mwp, y = ilfm)) +
  geom_point(aes(color = species), alpha = 0.8, size = .7) +  
  geom_line(data = model_lines, aes(x = mwp, y = ilfm_pred, color = species), size = 0.5) +  
...
figSXb <- ggplot(concept_slopes_df , aes(x = mwp, y = lfm*100)) +
  geom_point(aes(color = species), alpha = 0.8, size = .7) +  
  geom_line(data = model_lines, aes(x = mwp, y = 100*(1/ilfm_pred), color = species), size = .5) +  
  color_species_selected + 
  labs(
    x = expression(Psi[MDmin]~"(MPa)"),
    y = "LFM (%)"
...
legend <- cowplot::get_legend(
  ggplot(concept_slopes_df, aes(x = mwp, y = ilfm, color = species)) +
    geom_line(size = 0.5) +
    labs(color = "Species") +
    theme_minimal() +
    color_species_selected
...
# Combine the two square plots side-by-side
fig_panels <- cowplot::plot_grid(figSXb, figSXa, ncol = 2, rel_widths = c(1, 1))

# Combine panels with legend and adjust label margins so they don't overlap
figSX <- cowplot::plot_grid(fig_panels, legend, ncol = 2, rel_widths = c(1, 0.2)) +
...
# Height lowered slightly to perfectly match the square aspect ratio geometry
ggsave(plot = figSX, here::here("figures", "figSX_selectspp.jpg"), dpi = 600, height = 3.5, width = 8, units = "in")
```
#----------
...
fig3a <- ggplot(concept_df %>%
  filter(group == "grand"), aes(x = mwp, y = ilfm)) +
  geom_point(aes(color = species), alpha = 0.3, color = "black") +  
  geom_line(data = model_lines_solid, aes(x = mwp, y = ilfm_pred, color = study, group = interaction(species, study)), size = 0.5) +  
  geom_line(data = model_lines_dashed, aes(x = mwp, y = ilfm_pred, color = study, group = interaction(species, study)), size = 0.5, linetype = "dashed") +  
  labs(
...
fig3b <- ggplot(concept_df %>%
  ...
  geom_point(aes(color = species), alpha = 0.3, color = "black") +  
  geom_line(data = model_lines_solid, aes(x = mwp, y = 1 / ilfm_pred, color = study, group = interaction(species, study)), size = 0.5) +  
  geom_line(data = model_lines_dashed, aes(x = mwp, y = 1 / ilfm_pred, color = study, group = interaction(species, study)), size = 0.5, linetype = "dashed") +  
  labs(
...
model_lines <- concept_df %>%
  filter(group == "grand") %>% 
  distinct(species, intercept, slope) %>% 
  group_by(species) %>%
  reframe(
    mwp = seq(min(concept_df$mwp, na.rm = TRUE), max(concept_df$mwp, na.rm = TRUE), length.out = 100), 
    ilfm_pred = intercept + slope * mwp,  
...
fig3b <- ggplot(concept_df %>%
  filter(group == "grand"), aes(x = mwp, y = lfm)) +
  geom_point(aes(color = species), alpha = 0.5) +  
  geom_line(data = model_lines, aes(x = mwp, y = 1/ilfm_pred, color = species), size = .5) +  
  labs(
    x = "Water Potential (MWP)",
    y = "LFM",
    color = "Study"
...
figSXa <- ggplot(concept_slopes_df, aes(x = mwp, y = ilfm)) +
  ...
  geom_line(
    ...
    aes(x = mwp, y = ilfm_pred, color = functional_type, group = species),
    linewidth = 0.5
  ) +
  geom_line(
...
figSXb <- ggplot(concept_slopes_df, aes(x = mwp, y = lfm)) +
  geom_point(aes(color = functional_type), alpha = 1, size = 1) +
  geom_line(
    data = model_lines,
    aes(x = mwp, y = 1 / ilfm_pred, color = functional_type, group = species),
    linewidth = 0.5
  ) +
  geom_line(
    data = dplyr::filter(model_lines, functional_type == "Angiosperm"),
...
legend <- cowplot::get_legend(
  ggplot(model_lines, aes(x = mwp, y = 1 / ilfm_pred,
...
figSXa <- ggplot(concept_slopes_df, aes(x = mwp, y = ilfm)) +
  ...
  geom_line(
    data = model_lines,
    aes(x = mwp, y = ilfm_pred, color = genus, group = species),
    linewidth = 0.5
  ) +
  labs(
    x = "Water Potential (MWP)",
...
figSXb <- ggplot(concept_slopes_df, aes(x = mwp, y = lfm)) +
  ...
  geom_line(
    data = model_lines,
    aes(x = mwp, y = 1 / ilfm_pred, color = genus, group = species),
    linewidth = 0.5
  ) +
  labs(
    x = "Water Potential (MWP)",
...
figSXa <- ggplot(concept_slopes_df, aes(x = mwp, y = ilfm)) +
  ...
  geom_line(
    data = model_lines,
    aes(x = mwp, y = ilfm_pred, color = family, group = species),
    linewidth = 0.5
  ) +
  labs(
    x = "Water Potential (MWP)",
...
figSXb <- ggplot(concept_slopes_df, aes(x = mwp, y = lfm)) +
  ...
  geom_line(
    data = model_lines,
    aes(x = mwp, y = 1 / ilfm_pred, color = family, group = species),
    linewidth = 0.5
  ) +
  labs(
    x = "Water Potential (MWP)",
...
figSXa <- ggplot(concept_slopes_df, aes(x = mwp, y = ilfm)) +
  ...
  geom_line(
    data = model_lines,
    aes(x = mwp, y = ilfm_pred, color = study, group = species),
    linewidth = 0.5
  ) +
  labs(
    x = "Water Potential (MWP)",
...
figSXb <- ggplot(concept_slopes_df, aes(x = mwp, y = lfm)) +
  ...
  geom_line(
    data = model_lines,
    aes(x = mwp, y = 1 / ilfm_pred, color = study, group = species),
    linewidth = 0.5
  ) +
  labs(
    x = "Water Potential (MWP)",
