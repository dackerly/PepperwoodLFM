...
A model with no predictor variable (i.e. fitting the mean for all the height data) 
$$
\mathrm{height} \sim \mathrm{Normal}(\mu , \sigma)\\
\mu \sim \alpha \\
\alpha \sim \mathrm{Normal}(10,4) \\
\sigma \sim \mathrm{Exponential}(1) 
$$
```{r}
m.no.legs <- 
  quap( alist(YOURCODEEHRE
     ),
    data=d)
...
Now we consider a model where we use only one of the legs as a predictor of height.

$$
\mathrm{height} \sim \mathrm{Normal}(\mu , \sigma)\\
\mu \sim \alpha + \beta_l * \mathrm{leg\_left} \\
\alpha \sim \mathrm{Normal}(10,4) \\
\beta_l \sim \mathrm{Normal}(0,2)\\
\sigma \sim \mathrm{Exponential}(1) 
$$
...
Do the same for the right leg as a predictor
```{r}
m.right <-
  YOURCODEEHRE
...
Now we want a model where we use both legs in the same model, i.e. a multiple regression.
...
1. First pick two rows from the posterior distribution and write down the values of b1 and b2
...
```{r}
mu_mean1 <- a_mean + b1*h1_leg1 + b2*h2_leg2
mu_mean1

mu_mean1 <- a_mean + b1v1*h1_leg1 + b2v1*h2_leg2
mu_mean1
```

5. Do they give similar results? Why?

 *They should give similar results for the predicted means, but not exactly the same* 
 
6. Now mix the left leg/right leg values from the data frame and repeat. Do they give similar results?

```{r}
mu_mean <- a_mean + b1*h2_leg2 + b2*h1_leg1
mu_mean
```

 *They will probably give different values* 
...
```{r}
sims.no.legs <- sim_df(m.no.legs, select(d,-index)) #remove index because sim_df also has a call called index (e.g. below example). 
#sim_df has a default of 1000 samples. For each individaul, it simulates the height 1000 times useing the parameters
...
A more complex model (that needs MCMC methods to fit) with both legs showing an unknown deviation from the true growth of legs value (i.e. measurement error or developmental noise)
